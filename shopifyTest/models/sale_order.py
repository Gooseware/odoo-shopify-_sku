...
class sale_order(models.Model):
        ...
        for instance in instances:
            ...
            for sale_order in sales_orders:
                ...
                for picking in picking_ids:
                    ...
                    for move in picking.move_lines.filtered(lambda x: x.sale_line_id.product_id.id == x.product_id.id and x.state == "done"):
                        ...
                        line_items = self.prepare_line_item_data(line_items, shopify_line_id, tracking_no, product_qty)
                    #Below code is used to manage the kit type product qty.
                    kit_sale_lines = picking.move_lines.filtered(lambda x: x.sale_line_id.product_id.id != x.product_id.id and x.state == "done").mapped('sale_line_id')
                    for kit_sale_line in kit_sale_lines:
                        ...
                        product_qty = int(kit_sale_line.product_qty)
                        tracking_no = picking.carrier_tracking_ref or False
                        line_items = self.prepare_line_item_data(line_items, shopify_line_id, tracking_no, product_qty)
    ...
    @api.model
    def prepare_line_item_data(self,line_items, shopify_line_id, tracking_no, product_qty):
        """ This method is used to prepare a line item data.
            @author: Haresh Mori @Emipro Technologies Pvt. Ltd on date 28 January 2021 .
            Task_id: 170362 - Status not updated due to some issue in fulfillment request/response
        """
